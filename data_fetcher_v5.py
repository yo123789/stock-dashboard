...
def fetch_index():
    ...
    try:
        ...
        if not sh.empty:
            ...
            return {"price": safe_float(r['最新价']), "change_pct": safe_float(r['涨跌幅']), "volume": round(vol, 1)}
    except: pass
    try:
    ...
    try:
        ...
        for i, (_, r) in enumerate(df.iterrows()):
            item = {
                ...
                "up_count": safe_int(r.get('上涨家数', 0)),
                "down_count": safe_int(r.get('下跌家数', 0)),
                "leader": str(r.get('领涨股', '')),
                "leader_pct": safe_float(r.get('领涨股-涨跌幅', 0)),
            }
            if i < 5:
                result["top5"].append(item)
            elif i >= len(df) - 3:
                result["bottom3"].append(item)
    except Exception as e:
...
def fetch_dragon_tiger():
    ...
    result = []
    try:
        df = ak.stock_lhb_detail_em(date=date_dash())
        if df is not None and not df.empty:
